...
// THIS IMPLEMENTATION IS RIPPED FROM CHISEL BUT DOES NOT USE SET SO THE GENERATED VERILOG IS CONSISTENT
...
/** Fibonacci Linear Feedback Shift Register (LFSR) generator.
  ...
  * $paramUpdateSeed
  */
class ConsistentFibonacciLFSR(
  width:         Int,
  taps:          Seq[Int],
  seed:          Option[BigInt] = Some(1),
  val reduction: chisel3.util.random.LFSRReduce = chisel3.util.random.XOR,
  step:          Int = 1,
  updateSeed:    Boolean = false
) extends  chisel3.util.random.PRNG(width, seed, step, updateSeed)
    with chisel3.util.random.LFSR {

  def delta(s: Seq[Bool]): Seq[Bool] = taps.sorted.map { case i => s(i - 1) }.reduce(reduction) +: s.dropRight(1)
...
/** A maximal period Fibonacci Linear Feedback Shift Register (LFSR) generator. The maximal period taps are sourced from
  ...
  * $paramReduction
  */
class ConsistentMaxPeriodFibonacciLFSR(width: Int, seed: Option[BigInt] = Some(1), reduction: chisel3.util.random.LFSRReduce = chisel3.util.random.XOR)
    extends ConsistentFibonacciLFSR(width, chisel3.util.random.LFSR.tapsMaxPeriod.getOrElse(width, chisel3.util.random.LFSR.badWidth(width)).head.toSeq, seed, reduction)
...
/** Utility for generating a pseudorandom [[UInt]] from a [[FibonacciLFSR]].
  ...
  * [[XNOR]])
  */
object ConsistentFibonacciLFSR {
  ...
  def apply(
    ...
    seed:      Option[BigInt] = Some(1),
    reduction: chisel3.util.random.LFSRReduce = chisel3.util.random.XOR
  ): UInt =  chisel3.util.random.PRNG(new ConsistentFibonacciLFSR(width, taps, seed, reduction), increment)
  ...
  def maxPeriod(
    ...
    seed:      Option[BigInt] = Some(1),
    reduction: chisel3.util.random.LFSRReduce = chisel3.util.random.XOR
  ): UInt =  chisel3.util.random.PRNG(new ConsistentMaxPeriodFibonacciLFSR(width, seed, reduction), increment)
