...
object SBM {
  ...
  def SBINIT_OUT_OF_RESET = BitPat(
    "b????????????????????????????????????????????????????????????????????????000000000000000000000000??????????10010001?????????10010",
  )
...
object SBMsgCompare {
  def apply(incMsg: UInt, refMsg: BitPat): Bool = {

    (incMsg(4, 0) === refMsg(4, 0)) &&          // opcode
    (incMsg(21, 14) === refMsg(21, 14)) &&      // msgcode
    (incMsg(39, 32) === refMsg(39, 32))         // msgsubcode
  }
}